...
%eg: 8 operating points gives 1,3,5...15 (2k+1 given k=0:7)
...
% 
% %set time constant for tuning goal (settle time should be 0.2,
% %so dividing by 5 should give approximate tau?
% tau = 0.1;
% %max percentage overshoot for step response
% max_overshoot = 10; 
% 
% goal = TuningGoal.StepTracking('delta_torque','yaw_rate',tau,max_overshoot);
% %connect the C0 controller to the plant state space model and tune the PID
% %gains for this plant
% 
% for k = 1:operating_points
%     [plants(:,:,k),C,gam,Info] = looptune(plants(:,:,k),C0,goal);
%     showTunable(C)
% end
% 
% %get wc from transfer function of each plant? DeltaT to yaw rate tf - bode
% %plot it and get where gain crosses zero
% 
